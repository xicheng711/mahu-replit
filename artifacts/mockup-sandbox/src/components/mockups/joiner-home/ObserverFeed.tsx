...
export function ObserverFeed() {
  ...
  const lockedTabs: Record<string, string> = {
    "✨": "打卡", "💊": "用药", "📔": "日记"
  };
  ...
  return (
    <div className="min-h-screen bg-[#FFF8F5] font-['Inter'] relative">
      ...
      {/* ── Announcement quick-action card ── */}
      <div className="mx-5 mb-3">
        <div className="bg-white rounded-2xl shadow-sm border border-rose-100 overflow-hidden">
          {/* Latest announcement */}
          <div className="px-4 pt-3 pb-2">
            <div className="flex items-center justify-between mb-2">
              <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">📢 家庭公告</p>
              <button
                onClick={() => setAnnounceModal(true)}
                className="text-xs font-semibold text-rose-500 bg-rose-50 border border-rose-200 px-3 py-1 rounded-full active:scale-95 transition-transform"
              >
                ＋ 发布公告
              </button>
            </div>
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-1.5 mt-1.5">
                    <div className="w-4 h-4 rounded-full bg-rose-200 flex items-center justify-center text-xs">👨</div>
                    <span className="text-xs text-slate-400 font-medium">小明 · 今天 14:02</span>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <button className="w-full px-4 py-2 text-xs text-rose-400 font-semibold border-t border-rose-50 text-center bg-rose-50/50">
            查看全部公告 →
          </button>
        </div>
      </div>

      {/* ── Elder status card — warm soft ── */}
          ...
          {feed.map((item, i) => (
            <div key={item.id} className="flex gap-3">
              <div className="flex flex-col items-center">
                <div className="w-8 h-8 rounded-full flex items-center justify-center text-sm flex-shrink-0" style={{ backgroundColor: item.bg, border: `2px solid ${item.color}25` }}>
                  <span>{item.icon}</span>
                </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-sm border-t border-slate-100 px-2 py-3 flex justify-around">
        {[
          ...
          <button
            key={t.l}
            onClick={() => t.locked && setLockedModal(t.l)}
          ...
          >
            ...
            {t.locked && (
              <span className="absolute -top-0.5 -right-0.5 w-3 h-3 bg-slate-300 rounded-full flex items-center justify-center" style={{ fontSize: 7, color: "white", lineHeight: 1 }}>🔒</span>
            )}
          </button>
        ))}
      </div>
      ...
      {lockedModal && (
        <div className="fixed inset-0 bg-black/40 flex items-end justify-center z-50" onClick={() => setLockedModal(null)}>
          <div className="bg-white rounded-t-3xl px-6 pt-6 pb-8 w-full max-w-sm" onClick={e => e.stopPropagation()}>
            <div className="w-10 h-1 bg-slate-200 rounded-full mx-auto mb-5" />
            <div className="text-center mb-4">
              <div className="w-16 h-16 rounded-2xl bg-slate-100 flex items-center justify-center text-3xl mx-auto mb-3">
                {lockedModal === "打卡" ? "✨" : lockedModal === "用药" ? "💊" : "📔"}
              </div>
              <p className="text-lg font-black text-slate-800">「{lockedModal}」功能</p>
              <p className="text-sm text-slate-500 mt-1.5 leading-relaxed">
                这是主要记录人专属功能。<br/>
                如果你也想使用完整的记录功能，<br/>可以创建自己的家庭档案成为主要照顾者。
              </p>
            </div>
            <div className="space-y-2.5">
              <button className="w-full bg-rose-500 text-white font-bold py-3 rounded-2xl text-sm active:scale-98">
                ＋ 创建我的家庭档案
              </button>
              <button
                onClick={() => setLockedModal(null)}
                className="w-full bg-slate-100 text-slate-500 font-semibold py-3 rounded-2xl text-sm"
              >
                稍后再说
              </button>
            </div>
      ...
      {announceModal && (
        <div className="fixed inset-0 bg-black/40 flex items-end justify-center z-50" onClick={() => { setAnnounceModal(false); setPosted(false); setNewAnnounce(""); }}>
          <div className="bg-white rounded-t-3xl px-6 pt-6 pb-8 w-full max-w-sm" onClick={e => e.stopPropagation()}>
            <div className="w-10 h-1 bg-slate-200 rounded-full mx-auto mb-5" />
            {posted ? (
              <div className="text-center py-4">
                ...
                <button onClick={() => { setAnnounceModal(false); setPosted(false); setNewAnnounce(""); }} className="mt-4 bg-rose-500 text-white font-bold py-3 px-8 rounded-2xl text-sm">好的</button>
              </div>
            ) : (
              <>
                <p className="text-lg font-black text-slate-800 mb-1">发布家庭公告</p>
                ...
                <textarea
                  ...
                  className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-700 resize-none outline-none focus:border-rose-300"
                  rows={4}
                />
                <div className="flex gap-2 mt-3">
                  {["📢 通知", "🏥 就医", "💊 用药", "🌿 日常"].map(tag => (
                    <button key={tag} className="text-xs bg-slate-100 text-slate-600 px-2.5 py-1 rounded-full">{tag}</button>
                  ))}
                </div>
                <button
                  onClick={() => newAnnounce.trim() && setPosted(true)}
                  className={`w-full mt-4 font-bold py-3 rounded-2xl text-sm transition-all ${newAnnounce.trim() ? "bg-rose-500 text-white" : "bg-slate-100 text-slate-400"}`}
                >
                  发布公告
                </button>
              </>
